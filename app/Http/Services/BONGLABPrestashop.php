...
class BONGLABPrestashop implements PrestashopApiInterface
{
	use SMTPService;

    const PS_KEY = 'UJWRIG3251NR2JNNH69589FABXGRPROD';
    const PS_URL = 'https://dlds.cl';
	...
	protected $id_customer = 2687; //Bonglab Fulfillment
   ...
   public function postPrestashopOrder(array $productsToFulfill, array $ordersIdsToFulfill): void
    {
		...
		try {
			...
			echo "<br>ORDER ID: ".$newOrderId."<br><br>";

			// $this->verifyOrderPlacedProductsAndStatus($productsToFulfill, $newOrderId);
		  //  $this->verifyStatusOrderPlacedProductsAndStatus($productsToFulfill, $newOrderId);
			...
			\DB::connection('bonglab')->table('orders_fulfillment')
			...
			\DB::connection('bonglab')->table('orders_fulfillment_log')
			->whereIn('id_order_astro', $ordersIdsToFulfill)
			->update(['mensaje' => 'En espera']);
			
			
			\DB::connection('bonglab')->table('x4t2_orders')
    		->whereIn('id_order', $ordersIdsToFulfill)
    		->update(['id_order_dlds' => 'En espera']);
			...
			\DB::connection('bonglab')->table('orders_fulfillment')->delete();

        }
    }

    protected function setSyncBatchCode(){
		$resultado = \DB::connection('bonglab')->select('SELECT batch_code FROM orders_fulfillment_log ORDER BY id DESC LIMIT 1');
	...
	*/
	...
	protected function createOrder($productsToFulfill, $cartId)
	{
		...
		foreach ($productsToFulfill as $productFulfill){
		    ...
		    if($specific_price) $priceReduction = (float)$specific_price->reduction;

			//we need to get the specific_prices resource for this product 
			//because has special prices from dlds
			...
			foreach ($specificPriceRules->specific_prices->specific_price as $specificPriceIds){
				...
				// we know that Group 5 is the Franquicias group in DLDS, which is the one that belongs to
        ...
        Log::warning(json_encode($productsToFulfill));
        
        foreach($productsToFulfill as $product){
    
           $stock = \DB::connection('dlds')->table('r3pa_stock_available')->select('quantity')->where('id_product', (int)$productFulfill->id_producto_dlds)->first();
           
           if($product->qty_producto_total > ($stock->quantity - 3)){
	...
	public function validateStock($products){
	    ...
	    foreach($products as $productFulfill){
	        ...
	        $product = $this->psWebService->get(array('url' => $this::PS_URL . '/api/stock_availables/' . (int)$productFulfill->id_producto_dlds));
	        
	        error_log(json_encode($productFulfill));
				
    		if((int)$product->product_quantity < (int)$productFulfill->qty_producto_total){
	...
	public function validateStockID($id){
	    
	        
	         $stock = \DB::connection('dlds')->table('r3pa_stock_available')->select('quantity')->where('id_product', (int)$id)->first();
	        
	        echo json_encode($stock->quantity - 3);
	...
	{
		$resultado = \DB::connection('bonglab')->select("SELECT id FROM sync_orders_fulfillment WHERE sku_producto = '$sku' AND id_order = $id_order");
	...
	protected function createLog($ordersIdsToFulfill, $newOrderId = null, $mensaje = '')
	{
		...
		$ordersToFulfillData = \DB::connection('bonglab')->select('SELECT * FROM orders_fulfillment WHERE id_order IN(' . implode(',', $ordersIdsToFulfill). ')');
		...
		\DB::connection('bonglab')->insert($queryInsertLog);
	}
	...
	protected function updateLog($ordersIdsToFulfill, $newOrderId)
	{
	    ...
	    if($newOrderId){
	        ...
	        if($newOrderId < 0){
        	    ...
        	    \DB::connection('bonglab')->table('x4t2_orders')
        		->whereIn('id_order', $ordersIdsToFulfill)
        		->update(['id_order_dlds' => "En espera por stock"]);
	  ...
	  $carrier = \DB::connection('bonglab')->table('x4t2_orders')->select('x4t2_carrier.name as carrier')
    			                        ->whereIn('id_order', $ordersIdsToFulfill)
    			                        ->join('x4t2_carrier', 'x4t2_carrier.id_carrier', '=', 'x4t2_orders.id_carrier')
    			                        ->first(); 
			    
	  \DB::connection('bonglab')->table('x4t2_orders')
	  ->whereIn('id_order', $ordersIdsToFulfill)
	  ->update(['carrier_name' => $carrier->carrier ]);
	}

	protected function deleteOrdersThatWereFulfilled($ordersIdsToFulfill){
		\DB::connection('bonglab')->table('orders_fulfillment')->whereIn('id_order', $ordersIdsToFulfill)->delete();
 		\DB::connection('bonglab')->table('orders_fulfillment')->delete();
	}
